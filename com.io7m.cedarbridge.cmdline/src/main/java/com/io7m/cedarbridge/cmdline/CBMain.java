...
public final class CBMain implements Runnable
{
  private static final Logger LOG = LoggerFactory.getLogger(CBMain.class);
    ...
    final String[] inArgs)
  {
    ...
    final var metadata =
      new QApplicationMetadata(
        ...
        CBVersion.MAIN_VERSION,
        CBVersion.MAIN_BUILD,
        "The cedarbridge command-line application.",
        Optional.of(URI.create("https://www.io7m.com/software/cedarbridge/"))
      );

    final var builder = QApplication.builder(metadata);
    builder.addCommand(new CBCommandCheck());
    builder.addCommand(new CBCommandCompile());
    builder.addCommand(new CBCommandDocument());
    builder.addCommand(new CBCommandListCodeGenerators());
    builder.addCommand(new CBCommandListDocGenerators());
  ...
  public static int mainExitless(
    final String[] args)
  {
    final CBMain cm = new CBMain(args);
    cm.run();
    return cm.exitCode();
  }
